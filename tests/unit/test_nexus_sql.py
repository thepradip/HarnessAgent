"""Unit tests for NexusSql — standalone SQL agent with self-correction."""
...
async def test_ariasql_schema_context_from_store(tmp_path):
    ...
    agent = NexusSql(llm_provider=_MockLLM(), schema_store=store)
...
async def test_generate_sql_no_correction_when_high_score():
    ...
    agent = NexusSql(llm_provider=llm, verifier=verifier, correction_threshold=0.60)
...
async def test_generate_sql_with_schema_store_and_db(tmp_path):
    ...
    agent = NexusSql(llm_provider=llm, schema_store=store)
...
async def test_generate_sql_llm_failure_returns_fallback():
    ...
    agent = NexusSql(llm_provider=FailLLM())
    sql = await agent.generate_sql("count employees")
    assert sql == "SELECT 1"  # fallback


# ===========================================================================
# NexusSql — no verifier path
# ===========================================================================

@pytest.mark.asyncio
async def test_generate_sql_no_verifier_no_retries():
    llm = _MockLLM("SELECT 1")
    agent = NexusSql(llm_provider=llm, verifier=None)
...
@pytest.mark.asyncio
async def test_nexus_sql_agent_preload_schema(tmp_path):
    from harness.agents.nexus_sql import NexusSqlAgent

    db = _sqlite_db(tmp_path)
    store = _schema_store()

    agent = NexusSqlAgent.__new__(NexusSqlAgent)
...
@pytest.mark.asyncio
async def test_nexus_sql_agent_relevant_tables_small_db(tmp_path):
    from harness.agents.nexus_sql import NexusSqlAgent
...
@pytest.mark.asyncio
async def test_nexus_sql_agent_relevant_tables_large_db(tmp_path):
    from harness.agents.nexus_sql import NexusSqlAgent
...
def test_nexus_sql_agent_build_system_prompt_contains_schema():
    from harness.agents.nexus_sql import NexusSqlAgent
...
def test_nexus_sql_agent_build_system_prompt_no_schema():
    from harness.agents.nexus_sql import NexusSqlAgent
...
async def test_ariasql_mock_fallback_when_no_llm():
    """When no LLM is configured, NexusSql falls back to SELECT 1."""
    class FailLLM:
        async def complete(self, messages, **kwargs):
            raise RuntimeError("no api key")

    agent = NexusSql(llm_provider=FailLLM(), verifier=None)
